...
import numpy as np
import os
from router import ubidot, db_conn
...
class Camera(object):
    def __init__(self):
        ...
        self.names = ['youngil', 'wonjae', 'dongho', 'byungchan','donggeon']
    ...
    def get_frame(self, collection, ubi_data):
...
def gen(camera):
    collection = db_conn.mongoConn()
    ubi_datas = ubidot.ubi_connect()

    while True:
        frame = camera.get_frame(collection, ubi_datas)
        if frame == -1:
            break
